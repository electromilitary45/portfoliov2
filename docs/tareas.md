...
- [x] **2026-08-25 — Formulario de contacto funcional**: formulario en `/contactame` (honeypot + validación server-side) con doble canal: email vía **Resend** (`RESEND_API_KEY`, plantilla HTML propia) + respaldo en tabla `contact_messages` (**migración 014**). Bandeja nueva `/admin/mensajes` en el CMS (leer/no leer, eliminar, contadores). Env vars nuevas documentadas. ⚠️ Aplicar migración 014 en Supabase.
